...
> **Note:** The following article is reproduced verbatim from  
> Product Compass Team, *Product Compass* (2025):  
> [Interactive RAG Simulator For PMs & AI Engineers](https://rag.productcompass.pm/)  
> for internal educational use only (non-profit).

# Interactive RAG Simulator For PMs & AI Engineers

### Step-by-step operations in retrieval-augmented generation processes. Simple RAG, Agentic RAG, CAG, HyDE.

This interactive simulator provides hands-on experience with different RAG architectures and processes. It covers:

- **Simple RAG**: Basic retrieval-augmented generation
- **Agentic RAG**: RAG with AI agent capabilities
- **CAG**: Conversational AI with RAG
- **HyDE**: Hypothetical Document Embeddings

The simulator allows PMs and AI engineers to understand the step-by-step operations involved in RAG processes through interactive examples and visualizations.
...
### Updated: Aug 5, 2025. Learn RAG, develop a better AI product intuition, and build an AI-powered chatbot for your PM portfolio. A step-by-step guide with ready-to-use templates.
...
2. How to Build an AI-Powered RAG Chatbot Step-by-Step
   ...
   - Step 2: Store new documents in your vector database (n8n template)
   - Step 3: Create an n8n workflow that answers user requests (2 n8n templates):
     - Option 1: Vanilla RAG
     - Option 2: Standard RAG
   - Step 4: Use Lovable to create a simple RAG chatbot

## What is RAG and Why We Need It

RAG combines the power of retrieval systems with generative AI to provide more accurate and up-to-date responses by accessing external knowledge sources.

## How to Build an AI-Powered RAG Chatbot Step-by-Step

### Step 1: Create a vector database

Start by setting up a vector database to store document embeddings.

### Step 2: Store new documents in your vector database

Use n8n templates to automate the process of storing documents in your vector database.

### Step 3: Create an n8n workflow that answers user requests
...
**Option 2: Standard RAG**
Enhanced RAG with additional processing and optimization.

### Step 4: Use Lovable to create a simple RAG chatbot

Leverage Lovable's no-code platform to build the final chatbot interface.

This approach allows PMs to build practical RAG applications without deep technical knowledge, focusing on product intuition and user experience.

> **Note:** The following article is reproduced verbatim from  
> Bhavishya Pandit, *Bhavishya Pandit* (2025):  
> [Multimodal Retrieval-Augmented Generation (RAG)](https://bhavishyapandit9.substack.com/p/multimodal-retrieval-augmented-generation)  
> for internal educational use only (non-profit).
...
> [Evaluating RAG Pipelines](https://neptune.ai/blog/rag-pipeline)  
> for internal educational use only (non-profit).
...
3. The retrieved documents are passed to the generator (typically an LLM) along with the original query.

4. The generator produces a response based on the query and the retrieved context.

## Dimensions of RAG Evaluation

RAG evaluation should be approached across three dimensions:

### Performance
- **Retrieval metrics**: Recall@k, Precision@k, Mean Reciprocal Rank (MRR)
- **Generation metrics**: F1 score, BLEU, ROUGE, semantic similarity
- **End-to-end metrics**: Human evaluation, relevance scores

### Cost
- **Computational cost**: Token usage, API calls, processing time
- **Storage cost**: Vector database size, embedding storage
- **Maintenance cost**: Model updates, data refresh cycles

### Latency
- **Retrieval latency**: Time to fetch relevant documents
- **Generation latency**: Time for LLM to produce response
- **Total latency**: End-to-end response time

## Why is RAG Pipeline Evaluation Challenging?

RAG evaluation is challenging for several reasons:

1. **Multiple components**: Each stage (retrieval, generation, post-processing) requires different evaluation approaches
2. **Human judgment**: Traditional metrics often fail to capture nuanced human assessment of quality
3. **Context dependency**: Performance varies significantly based on the specific use case and domain
4. **Evolving benchmarks**: The field is rapidly evolving with new evaluation methodologies
5. **Resource constraints**: Comprehensive evaluation requires significant computational and human resources

## How to Evaluate a RAG Pipeline

### Curating an Evaluation Dataset

The foundation of any evaluation is a high-quality dataset that represents your use case. This should include:

- **Diverse queries**: Cover different types of questions and user intents
- **Ground truth**: Correct answers or expected responses for each query
- **Context documents**: The knowledge base that should be retrieved
- **Edge cases**: Queries that test the system's limits and robustness
...
- **Hit rate**: Whether at least one relevant document is in top-k results

#### Rank-based metrics
- **Mean Reciprocal Rank (MRR)**: Average of reciprocal ranks of first relevant document
- **Normalized Discounted Cumulative Gain (NDCG)**: Considers both relevance and ranking order
- **Mean Average Precision (MAP)**: Average precision across all queries

### Evaluation of the Generator

#### Citation-based metrics
- **Citation accuracy**: Whether the generated response correctly cites retrieved documents
- **Citation relevance**: Whether cited documents are actually relevant to the query
- **Citation completeness**: Whether all relevant information is properly cited

#### Response-based metrics
- **F1 score at token level**: Measures overlap between generated and reference text
- **Semantic similarity**: Cosine similarity between generated and reference embeddings
- **Faithfulness**: Whether the response accurately reflects the retrieved context
- **Answerability**: Whether the response actually answers the query
...
End-to-end evaluation assesses the complete RAG pipeline:
...
The optimization of a RAG pipeline can be divided into three stages:
...
##### 1. Chunking Strategy
- **Semantic chunking**: Split documents based on meaning rather than fixed size
...
##### 2. Chunk Size
- **Small chunks**: Better for precise retrieval but may lose context
- **Large chunks**: Maintain context but may include irrelevant information
- **Optimal size**: Typically 256-512 tokens, depending on use case

##### 3. Metadata
- **Rich metadata**: Include source, date, author, category, etc.
- **Structured information**: Use JSON or similar format for easy filtering
- **Embedding metadata**: Include metadata in embedding generation

#### Optimizing the user query

##### Query rewriting
- **Query expansion**: Add synonyms and related terms
- **Query reformulation**: Restructure complex queries
- **Query clarification**: Ask for clarification when queries are ambiguous

##### Adding context to the query
- **Conversation history**: Include previous exchanges
- **User profile**: Consider user preferences and history
- **Domain context**: Add relevant domain-specific information

##### Simplifying overly long queries
- **Query summarization**: Extract key points from long queries
- **Query decomposition**: Break complex queries into simpler parts
- **Focus extraction**: Identify the main intent of the query

##### Query routing
- **Intent classification**: Route queries to appropriate subsystems
- **Domain routing**: Direct queries to relevant knowledge bases
- **Complexity routing**: Route simple vs. complex queries differently

#### Optimizing the vector database

##### Choosing the right vector database
- **Pinecone**: Good for production deployments
- **Weaviate**: Excellent for complex queries and metadata filtering
- **Chroma**: Great for development and prototyping
- **Qdrant**: Good balance of features and performance

##### Embedding model selection
- **Domain-specific models**: Use models trained on your domain
- **Multilingual models**: Consider if you need multiple languages
- **Model size**: Balance between performance and cost
- **Update frequency**: Consider how often you need to update embeddings
...
- **Dense retrieval**: Use embedding similarity (most common)
- **Sparse retrieval**: Use keyword matching (BM25, TF-IDF)
- **Hybrid retrieval**: Combine dense and sparse methods
- **Multi-vector retrieval**: Use multiple embeddings per document

##### Apply re-ranking
- **Cross-encoder models**: Re-rank retrieved documents using more sophisticated models
- **Multi-criteria ranking**: Consider relevance, freshness, authority
- **Personalized ranking**: Adapt ranking to user preferences
...
- **Context window**: Ensure sufficient context for complex queries
- **Model fine-tuning**: Fine-tune on your specific domain

##### Design better prompts
- **Clear instructions**: Provide explicit guidance to the model
- **Few-shot examples**: Include examples of good responses
- **Context formatting**: Structure retrieved context clearly
...
- **Citation verification**: Ensure citations are accurate and relevant
- **Hallucination detection**: Identify and flag potentially false information

#### Response enhancement
- **Formatting**: Ensure consistent and readable output
- **Summarization**: Provide concise summaries when appropriate
...
- **Compliance checking**: Ensure adherence to relevant regulations

## Final Thoughts

Evaluating and optimizing RAG pipelines is a complex but essential task. The key is to:

1. **Start with clear objectives**: Define what success looks like for your use case
2. **Use multiple metrics**: Don't rely on a single evaluation approach
3. **Iterate continuously**: RAG systems require ongoing optimization
4. **Consider the full pipeline**: Optimize each component while maintaining end-to-end performance
5. **Monitor in production**: Track real-world performance and user feedback

Remember that RAG evaluation is not a one-time task but an ongoing process. As your knowledge base grows, user needs evolve, and new techniques emerge, you'll need to continuously evaluate and improve your RAG pipeline.
